...
void GameScene  :: renderScene() {
  ...
  for (int i  = 0; i < numAvatar;  ++i) {
    ...
    Serial.printf("renderableAvatar[%d] minX,minY: [%d, %d] maxX,maxY:[%d, %d] \n" , numRenderableAvatar, renderableMinx[numRenderableAvatar], renderableMiny[numRenderableAvatar], renderableMaxx[numRenderableAvatar], renderableMaxy[numRenderableAvatar] );
#endif

  }

#ifdef DEBUG_RENDERSCENE
  Serial.printf("Search for Discrete drawing area; total # of renderable avatars: [%d] \n", numRenderableAvatar);
#endif
...
#ifdef DEBUG_RENDERSCENE
        //        String debugMsg2 = "Renderable # " + j + " not overlap, check next" + numRenderableAvatar;
        //        Serial.println(debugMsg2);
#endif
        continue;
      } else {
...
void GameScene::drawAvatar2Buffer(Avatar *avatar, uint16_t* destPtr, uint16_t y) {
  ...
  //Serial.println(avatar->currentFrame);
